...
class QuantLinear(nn.Linear):
    ...
    def forward(self, x):
        ...
        if self.use_input_quant:
            # UniformQuantizer's channel_wise=True path expects 3D (B, N, C) so the
            # last dim is the feature axis. EVA's ViT passes 4D (B, H, W, C) into Linear,
            # so flatten (H, W) -> N before quantising and restore shape afterwards.
            orig_shape = x.shape
            if x.dim() == 4:
                B, H, W, C = orig_shape
                x = x.reshape(B, H * W, C)
...
class MatMul(nn.Module):
    """Identity wrapper for A @ B. Giving the matmul a named module lets
    quant_model_eva() locate and replace it with QuantMatMul."""
    def forward(self, A, B):
        return A @ B
