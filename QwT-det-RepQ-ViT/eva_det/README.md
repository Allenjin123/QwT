...
> **Note (Blackwell GPUs, e.g. B100/B200/RTX 5090):** `mmcv-full==1.6.1` cannot be built on Blackwell (no prebuilt wheels, and the source tree targets CUDA ≤12.1 / PyTorch ≤2.3, so nvcc fails on `sm_100`/`sm_120`). We therefore **did not install mmcv** in our setup, which means **soft-NMS is unavailable** — the `model.roi_heads.use_soft_nms=True` flag below will fall back to standard NMS, and the reported AP will be ~0.2–0.5 points lower than the numbers in the tables.
